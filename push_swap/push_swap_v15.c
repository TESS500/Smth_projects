...
int		ft_finder_a(int *A, int x, int z, int *in_a, int ac_a)
{
	int start;
	int end;
	int index;
	...
	*in_a = 0;
	...
	while (z - index > x)
	{
		...
		while (start <= end)
		{
			if (z > A[start] && x < A[start])
			{
				*in_a = start;
				return (1);
			}
			else if (z > A[end] && x < A[end])
			{
				*in_a = end;
				return (1);
			}
...
if (key == 0)
	{
		if ((a_pos <= ac_a / 2 && ac_a >= ac_b)
			|| (b_pos <= ac_b / 2 && ac_a <= ac_b))
			ft_slide_rr(A, B, a_pos, b_pos, ac_a, ac_b, 0, x_operations);
		else if ((a_pos > ac_a / 2 && ac_a >= ac_b)
				|| (b_pos > ac_b / 2 && ac_a <= ac_b))
    ...
    //         // || (a_pos <= ac_a / 2 && a_pos >= b_pos)
    //         // || (b_pos <= ac_b / 2 && b_pos >= a_pos))
	// 		// || (a_pos <= ac_a / 2 && ac_a >= ac_b)
    //         // || (b_pos <= ac_b / 2 && ac_b >= ac_a))
	// 		ft_slide_rr(A, B, a_pos, b_pos, ac_a, ac_b, 0, x_operations);
...
int		ft_is_miss_counter(int *l, int *m, int *count_of_steps, int *i, int *ii, int *iii, int ac_a, int ac)
{
...
void	ft_is_missposition(int	*A, int *B, int *ac_a, int *ac_b, int ac, int *first, int *x_operations)
{
    ...
    ii = A[0];
	quat = 3;
	one_of_four = ac / (quat + 1);

	while (ft_stack_not_sorted(A, ac))
	{
		while (*ac_b < one_of_four)
		{
			ft_finder_a(A, quat * one_of_four - 1, (quat + 1) * one_of_four, &in_a, *ac_a); // in A
			// if (A[0] >= quat * one_of_four && A[0] < (quat + 1) * one_of_four)
			// {
			if (ft_finder_b_to_up(B, A[in_a], ac, &in_b, *ac_b))
			{
				ft_slide_a_b(A, B, in_a, in_b, *ac_a, *ac_b, 1, x_operations);
				ft_pb(A, B, ac, ac_a, ac_b, x_operations);
			}
			else if (ft_finder_b(B, -1, A[in_a], &in_b, *ac_b) || *ac_b == 0)
			{
			...
			// else
			// 	ft_rra(A, *ac_a, 1, x_operations);
			ft_print_mass_a_b(A, B, ac); // print MASS
		}
		ft_finder_b(B, (quat + 1) * one_of_four - 2, (quat + 1) * one_of_four, &in_b, *ac_b);
		quat--;
		ft_finder_a(A, quat * one_of_four - 1, (quat + 1) * one_of_four, &in_a, *ac_a);
		ft_slide_a_b(A, B, in_a, in_b, *ac_a, *ac_b, 0, x_operations);
		ft_print_mass_a_b(A, B, ac); // print MASS
		while (*ac_b < one_of_four * 2)
		{
			ft_finder_a(A, quat * one_of_four - 1, (quat + 1) * one_of_four, &in_a, *ac_a);
			ft_slide_a_b(A, B, in_a, 0, *ac_a, *ac_b, 0, x_operations);
			ft_pb(A, B, ac, ac_a, ac_b, x_operations);
		}
		ft_print_mass_a_b(A, B, ac); // print MASS
		ft_finder_b(B, (quat + 2) * one_of_four - 2, (quat + 2) * one_of_four, &in_b, *ac_b);
		ft_finder_b(A, (quat + 2) * one_of_four - 1, (quat + 2) * one_of_four + 1, &in_a, *ac_a);
		ft_slide_a_b(A, B, in_a, in_b, *ac_a, *ac_b, 0, x_operations);
		ft_print_mass_a_b(A, B, ac); // print MASS
		while (*ac_b > one_of_four)
			ft_pa(A, B, ac, ac_a, ac_b, x_operations);
		...
		while (*ac_b)
		{
			ft_finder_b(B, A[0] - 2, A[0], &in_b, *ac_b);
			ft_slide_a_b(A, B, 0, in_b, *ac_a, *ac_b, 0, x_operations);
			ft_pa(A, B, ac, ac_a, ac_b, x_operations);
		}
		quat--;
