...
// 	while (i < ac_b)
// 	{
// 		if ((x < B[i] && B[i] < z && B[i] != -1))
// 		{
// 			*in_b = i;
// 			return (1);
// 		}
// 		else if ((x < B[ac_b - 1] && B[ac_b - 1] < z && B[ac_b - 1] != -1))
// 		{
// 			*in_b = ac_b;
// 			return (1);
// 		}
...
// ------------------------------------------------------------------------------
void	ft_is_missposition(int	*A, int *B, int *ac_a, int *ac_b, int ac, int *x_operations)
{
	int i;
	int ii;
	int	iii;
	...
	m = *ac_a - 1;
	count_of_steps = 1;
	while (ft_is_miss_counter(&l, &m, &count_of_steps, &i, &ii, &iii, *ac_a, ac))
	{
		if (!(A[i] == A[ii] - 1) && !(A[i] == ac - 1 && A[ii] == 0))
		{
			quat = ft_diff_quaters(A[i], A[ii], ac);
			if ((A[i] > A[ii] && quat == 0)
					|| (A[i] < A[ii] && quat == -1)
					|| (A[i] == 0 && A[ii] == ac - 1)
					|| (A[i] - 1 == A[ii]))
			{
				ft_slide_a_b(A, B, i, 0, *ac_a, *ac_b, x_operations);
				ft_sa(A, 1, x_operations);
				return;
			}
			if (A[ii] - 1 == A[iii] || (A[ii] == 0 && A[iii] == ac - 1))
			{
				ft_slide_a_b(A, B, ii, 0, *ac_a, *ac_b, x_operations);
				ft_sa(A, 1, x_operations);
				return;
			}
			if (quat == 1)
			...
			{
				if (ft_finder_b(B, A[i], ac, &in_b, *ac_b))
				{
					ft_slide_a_b(A, B, ii, in_b, *ac_a, *ac_b, x_operations);
					ft_pa(A, B, ac, ac_a, ac_b, x_operations);
					return;
				}
				if (ft_finder_b(B, -1, A[ii], &in_b, *ac_b))
				{
					ft_slide_a_b(A, B, ii, in_b, *ac_a, *ac_b, x_operations);
					ft_pa(A, B, ac, ac_a, ac_b, x_operations);
					return;
			...
			{
				if (ft_finder_b(B, A[i], A[ii], &in_b, *ac_b))
				{
					ft_slide_a_b(A, B, ii, in_b, *ac_a, *ac_b, x_operations);
					ft_pa(A, B, ac, ac_a, ac_b, x_operations);
					return;
				}
				// if (ft_finder_b(B, A[i], ac, &in_b, *ac_b) && ac - A[i] < ac / 2)
				// {
				// 	ft_slide_a_b(A, B, i, in_b, *ac_a, *ac_b, x_operations);
				// 	ft_pa(A, B, ac, ac_a, ac_b, x_operations);
				// 	return;
				// }
				// if (ft_finder_b(B, A[ii], ac, &in_b, *ac_b))
				// {
				// 	ft_slide_a_b(A, B, ii, in_b, *ac_a, *ac_b, x_operations);
				// 	ft_pa(A, B, ac, ac_a, ac_b, x_operations);
				// 	return;
...
int		ft_stack_not_sorted(int *A, int ac)
{
	int		i;

	i = -1;
	while (++i < ac - 1)
		if (!(A[i] < A[i + 1] || (A[i] == ac - 1 && A[i + 1] == 0)) || A[i] == -1)
			return (1);
	return (0);
}
...
int		ft_sw_pu(int *A, int *B, int ac)
{
	...
	{
		ft_is_missposition(A, B, &ac_a, &ac_b, ac, &x_operations);
		ft_print_mass_a_b(A, B, ac);
	}
