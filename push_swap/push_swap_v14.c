...
void	ft_is_missposition(int	*A, int *B, int *ac_a, int *ac_b, int ac, int *x_operations)
{
    ...
    while (ft_stack_not_sorted(A, ac))
	{
		...
		while (*ac_a)
		{
			while (counter_steps < quat)
			{
				ft_finder_a(A, (tmp_ac - quat) - 1, tmp_ac, &in_a, *ac_a); // in A
		...
		while (*ac_b)
		{
			while (counter_steps < quat)
			{
				if (quat > 10)
				{
					ft_finder_a(B, (tmp_ac - quat) - 1, tmp_ac, &in_b, *ac_b); // in B
					ft_slide_a_b(A, B, 0, in_b, *ac_a, *ac_b, 0, x_operations);
					ft_pa(A, B, ac, ac_a, ac_b, x_operations);
					ft_print_mass_a_b(A, B, ac); // print MASS
				}
				...
				{
					ft_finder_b(B, ac - 2, ac, &in_b, *ac_b);
					ft_slide_a_b(A, B, 0, in_b, *ac_a, *ac_b, 0, x_operations);
					ft_pa(A, B, ac, ac_a, ac_b, x_operations);
				}
				else if (*ac_a < ac)
				{
					quat = 10;
					ft_finder_a(B, (tmp_ac - quat) - 1, tmp_ac, &in_b, *ac_b); // in B
					ft_finder_b_to_up(A, B[in_b], ac, &in_a, *ac_a);
