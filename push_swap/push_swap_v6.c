...
void	ft_is_missposition(int	*A, int *B, int *ac_a, int *ac_b, int ac, int *x_operations)
{
	...
	while (ft_is_miss_counter(&l, &m, &count_of_steps, &i, &ii, &iii, *ac_a, ac))
	{
		if (!(A[i] == A[ii] - 1) && !(A[i] == ac - 1 && A[ii] == 0))
		{
			...
			{
				ft_slide_a_b(A, B, ii, 0, *ac_a, *ac_b, 0, x_operations);
				ft_sa(A, 1, x_operations);
				return;
			}
			if (A[i] > A[ii] && A[ii] != 0)
			...
			{
                previous_nbr = A[i];
				if (ft_finder_b_to_up(B, A[ii], ac, &in_b, *ac_b))
						ft_slide_a_b(A, B, ii, in_b, *ac_a, *ac_b, 1, x_operations);
				else if (ft_finder_b(B, -1, A[ii], &in_b, *ac_b))
						ft_slide_a_b(A, B, ii, in_b, *ac_a, *ac_b, 0, x_operations);
				ft_pb(A, B, ac, ac_a, ac_b, x_operations);
                while (A[0] < previous_nbr && A[0] != 0)
                {
                    if (ft_finder_b_to_up(B, A[0], ac, &in_b, *ac_b))
						ft_slide_a_b(A, B, 0, in_b, *ac_a, *ac_b, 1, x_operations);
				    else if (ft_finder_b(B, -1, A[0], &in_b, *ac_b))
			...
			else if (A[i] < A[ii])
			{
                ...
                if (ft_finder_b(B, A[i], A[ii], &in_b, *ac_b))
                {
                    ft_slide_a_b(A, B, ii, in_b, *ac_a, *ac_b, 0, x_operations);
                    ft_pa(A, B, ac, ac_a, ac_b, x_operations);
                    while (B[0] > previous_nbr && B[0] < A[0] && B[0] != -1)
                    {
                        ft_pa(A, B, ac, ac_a, ac_b, x_operations);
                        // ft_print_mass_a_b(A, B, ac);
                    }
                ...
                // else if (ft_finder_b_to_up(B, A[i], A[ii], &in_b, *ac_b))
                // {
                //     ft_slide_a_b(A, B, ii, in_b, *ac_a, *ac_b, 1, x_operations);
                //     ft_pa(A, B, ac, ac_a, ac_b, x_operations);
                //     return;
