...
void	ft_is_missposition(int	*A, int *B, int *ac_a, int *ac_b, int ac, int *first, int *x_operations)
{
	...
	ft_print_mass_a_b(A, B, ac);
	while (ft_stack_not_sorted(A, *ac_a) || *ac_a < (ac / 2))
	{
		if (ft_finder_b(B, A[0] - 2, A[0], &in_b, *ac_b))
		{
			ft_slide_a_b(A, B, 0, in_b, *ac_a, *ac_b, 0, x_operations);
		...
		{
			ft_finder_b(A, A[0] - 2, A[0], &in_b, *ac_a);
			// if (in_b < *ac_a / 2)
			// {
			// 	ii = A[0];
			// 	ft_slide_a_b(A, B, in_b, 0, *ac_a, *ac_b, 0, x_operations);
			// 	ft_pb(A, B, ac, ac_a, ac_b, x_operations);
			// 	ft_finder_b(A, ii - 1, ii + 1, &in_b, *ac_a);
			// 	ft_slide_a_b(A, B, in_b, 0, *ac_a, *ac_b, 0, x_operations);
			// 	ft_pa(A, B, ac, ac_a, ac_b, x_operations);
			// }
			if (in_b < *ac_a / 2)
	...
	while (A[i] != ac / 2)
		i++;
	ft_finder_b(B, (ac / 2) - 2, ac / 2, &in_b, *ac_b);
