...
void	ft_is_missposition(int	*A, int *B, int *ac_a, int *ac_b, int ac, int *first, int *x_operations)
{
	...
	while (ft_stack_not_sorted(A, ac))
	{
		while (*ac_b < one_of_four)
		{
	        ...
	        m = *ac_a - 1;
	        count_of_steps = 1;
            while (!(A[i] >= quat * one_of_four && A[i] < (quat + 1) * one_of_four))
               ft_is_miss_counter(&l, &m, &count_of_steps, &i, &ii, &iii, *ac_a, ac);
            if (ft_finder_b_to_up(B, A[i], ac, &in_b, *ac_b))
            {
                ft_slide_a_b(A, B, i, in_b, *ac_a, *ac_b, 1, x_operations);
                ft_pb(A, B, ac, ac_a, ac_b, x_operations);
            }
            else if (ft_finder_b(B, -1, A[i], &in_b, *ac_b) || *ac_b == 0)
            {
		...
		ft_finder_b(A, (quat + 1) * one_of_four - 1, (quat + 1) * one_of_four + 1, &in_a, *ac_a);
