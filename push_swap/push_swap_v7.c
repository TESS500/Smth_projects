...
void	ft_is_missposition(int	*A, int *B, int *ac_a, int *ac_b, int ac, int *x_operations)
{
	...
	m = *ac_a - 1;
	count_of_steps = 1;
	while (ft_is_miss_counter_only_down(&l, &m, &count_of_steps, &i, &ii, &iii, *ac_a, ac))
	{
		if (!(A[i] == A[ii] + 1) && !(A[i] == 0 && A[ii] == ac - 1))
		{
			if ((A[i] == ac - 1 && A[ii] == 0)
					|| (A[i] + 1 == A[ii]))
			{
			...
			if (A[ii] + 1 == A[iii] || (A[ii] == ac - 1 && A[iii] == 0))
			{
				ft_slide_a_b(A, B, iii, 0, *ac_a, *ac_b, 0, x_operations);
				ft_sa(A, 1, x_operations);
				return;
			}
			previous_nbr = A[i];
			if (A[i] != 0)
			{
				if (ft_finder_b(B, A[i] - 2, A[i], &in_b, *ac_b))
                {
				...
				else
				{
					...
					if (count_difference * -1 < *ac_a / 2)
					{
						ft_slide_a_b(A, B, ii, in_b, *ac_a, *ac_b, 0, x_operations);
						while (A[0] != previous_nbr - 1)
						{
							if (A[0] == previous_nbr - 2)
								ft_rra(A, *ac_a, 1, x_operations);
							else if (ft_finder_b_to_up(B, A[0], ac, &in_b, *ac_b))
							{
								ft_slide_a_b(A, B, 0, in_b, *ac_a, *ac_b, 1, x_operations);
								ft_pb(A, B, ac, ac_a, ac_b, x_operations);
								ft_rra(A, *ac_a, 1, x_operations);
							}
							else if (ft_finder_b(B, -1, A[0], &in_b, *ac_b) || *ac_b == 0)
							{
					...
					{
						ft_slide_a_b(A, B, tmp_i, 0, *ac_a, *ac_b, 0, x_operations);
						ft_pb(A, B, ac, ac_a, ac_b, x_operations);
						tmp_i = 0;
						while (A[tmp_i] != previous_nbr)
							tmp_i++;
						ft_slide_a_b(A, B, tmp_i, 0, *ac_a, *ac_b, 0, x_operations);
						ft_pa(A, B, ac, ac_a, ac_b, x_operations);
						return;
