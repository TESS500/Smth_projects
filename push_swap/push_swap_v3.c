...
int		ft_finder_b(int *B, int x, int *in_b, int ac_b, int ac, int j)
{
	int i;
	int m;
	...
	*in_b = 0;
	...
	while (i <= m && i <= j)
	{
		...
		else if ((x == B[i] + 1 && B[i] != -1) || (x == ac - 1 && B[i] == 0))
		{
			*in_b = i;
			return (2);
		}
		else if ((x == B[m] - 1 && B[m] != -1) || (x == 0 && B[m] == ac - 1))
		{
			*in_b = m;
			return (1);
		}
		else if ((x == B[m] + 1 && B[m] != -1) || (x == ac - 1 && B[m] == 0))
		{
			*in_b = m;
			return (2);
		}
...
int		ft_finder_a(int	*A, int	*B, int *in_b_mark, int	*in_b, int ac_a, int ac_b, int ac, int *list_of_misspos)
{
	int	j;
	int	k;
	...
	k = ac_a - 1;
    *in_b_mark = 0;
	while (!list_of_misspos[A[j]] && !list_of_misspos[A[k]] && j < k)
	{	
		if (ac_b)
		{
			*in_b_mark = ft_finder_b(B, A[j], in_b, ac_b, ac, j);
			if (*in_b_mark)
				return (j);
			*in_b_mark = ft_finder_b(B, A[k], in_b, ac_b, ac, j);
			if (*in_b_mark)
				return (k);
		}
		j++;
...
void	ft_is_missposition(int	*A, int *B, int count_misspos, int *list_of_misspos, int to_rot, int ac, int *x_operations)
{
	...
	{
		j = ft_finder_a(A, B, &in_b_mark, &in_b, ac_a, ac_b, ac, list_of_misspos);
	...
	printf("!!!!!!!!!%d___%d___%d", count_misspos, to_rot, ac_b);
}
